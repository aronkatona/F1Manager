...
public class DriverDAOImpl implements DriverDAO {
	...
	public List<Driver> listDriversByTeamName(String teamName){
		Session session = this.sessionFactory.getCurrentSession();
		Query query = session.createQuery("from Team where name = :teamName");
		query.setParameter("teamName", teamName);
		List<Team> teams = query.list();
		int teamId = teams.get(0).getId();
		Query driversQuery = session.createQuery("from Driver where team_id = :teamId");
		driversQuery.setParameter("teamId", teamId);
		return driversQuery.list();
	}
